...
let TEXTS = [" FOLLOW THE WHITE RABBIT ", " WAKE UP "]
    ...
    async loadLoop(ind){
      ...
      // load for each loop
        ...
        if (loop_points.length > 2){
          ...
          var chosen_text = TEXTS[Math.floor(Math.random() * TEXTS.length)]
          var line_size = me.character_size * chosen_text.length;
          var repeat_time = Math.floor(l / (line_size* font_size));
          
          
          var damping_repeat = Math.random() * 0.8 + 0.2;

          while (Math.floor(repeat_time * damping_repeat) < 1 && l > 0){
          ...
          flowList.push(flow);
          flowSpeed.push((Math.random() > 0.5 ? -1 : 1)* (Math.random() * 3 + 0.5) * 0.002 / Math.log2(l + 2));
